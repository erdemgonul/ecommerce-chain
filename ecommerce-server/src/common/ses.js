...
exports.SendEmailWithTemplate = function (userOrAdresses, template, templateData, paramOptions = {}) {
  ...
  if (userOrAdresses && typeof userOrAdresses === 'string') { userOrAdresses = [userOrAdresses]; }

  if (templateData && typeof templateData === 'object') { templateData = JSON.stringify(templateData); }
  ...
  if (paramOptions) {
    if (paramOptions.FromEmailAddress) { params.FromEmailAddress = paramOptions.FromEmailAddress; }

    if (paramOptions.ReplyToAddresses) { params.ReplyToAddresses = [paramOptions.ReplyToAddresses]; }

    if (paramOptions.ContactListName) { params.ListManagementOptions.ContactListName = [paramOptions.ContactListName]; }

    if (paramOptions.TopicName) { params.ListManagementOptions.TopicName = [paramOptions.TopicName]; }
  }

  // Create the promise and SES service object

  const sendPromise = SESV2.sendEmail(params).promise();
